...
for sub in subjects:
    data_dir = f"/Users/student/PycharmProjects/data/derivatives/{sub}"
    ...
    if load_old_2:
        ...
        for split in sorted_keys:
            # Get list of the EVs and load them into a dictionary 
            if RDM_version == '01':
        ...
        if RDM_version in ['01']:
            ...
            for i, task in enumerate(sorted(reading_in_EVs_dict.keys())):
                if task not in ['ev_press_EV_EV_10']:
        ...
        # Creates the RDM matricies for each searchlight in the dataset
    ...
    # Step 3: load and compute the model RDMs (vector models, not matrix models)
    ...
    for model in data_dirs:
        # `prepare_model_data` takes the model data and turns it into an RDM object
        ...
        RDM_my_model_dir[model] = Parallel(n_jobs=3)(delayed(mc.analyse.analyse_MRI_behav.evaluate_model)(model = model_model, data = d) for d in tqdm(data_RDM, desc=f"running GLM for all searchlights in {model}"))
